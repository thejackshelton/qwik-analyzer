...
async function runIntegrationTests() {
	...
	for (const testCase of testCases) {
		...
		try {
			...
			if (result.hasComponent === testCase.expectedHasComponent) {
				console.log(
					`   ✅ PASSED - Expected: ${testCase.expectedHasComponent}, Got: ${result.hasComponent}`,
				);
				passedTests++;
			} else {
				console.log(
					`   ❌ FAILED - Expected: ${testCase.expectedHasComponent}, Got: ${result.hasComponent}`,
				);
			}
	...
	try {
		...
		console.log(
			`   ✅ Correctly handled non-existent file: ${(error as Error).message}`,
		);
	}
