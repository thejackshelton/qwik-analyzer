...
fn should_add_prop_to_component(element_name: &str, source_file_name: &str) -> bool {
    element_name.split('.').last().unwrap_or(element_name).to_lowercase() == source_file_name
}
...
pub fn transform_components(semantic: &Semantic, file_path: &Path) -> Result<Vec<Transformation>> {
    ...
    if let Some(transformation) = create_props_parameter_transformation(semantic, &source_text, file_path)? {
        transformations.push(transformation);
    }

    transformations.extend(create_component_present_call_transformations(semantic, file_path)?);
